...
class Strategy():
    ...
    def init_variable(self):
        ...
        self.__a_open_count_strategy = 0  # 本策略A的开仓手数
        self.__b_open_count_strategy = 0  # 本策略B的开仓手数
        ...
        self.__dict_statistics = {
            ...
            'a_open_count_strategy': self.__a_open_count_strategy,  # A开仓手数
            'b_open_count_strategy': self.__b_open_count_strategy,  # B开仓手数
    ...
    def statistics_for_order(self, Order):
        ...
        # print(">>>Strategy.statistics_for_order() user_id =", self.__user_id, "strategy_id =", self.__strategy_id)
        # print(" A委托手数 =", self.__a_order_lots)
        # print(" B委托手数 =", self.__b_order_lots)
        # print(" A委托次数 =", self.__a_order_times)
        # print(" B委托次数 =", self.__b_order_times)
    ...
    def statistics_for_trade(self, Trade):
        ...
        if Trade['InstrumentID'] == self.__a_instrument_id:
            ...
            if self.__a_order_lots > 0:
                self.__a_trade_rate = self.__a_traded_lots / self.__a_order_lots  # A成交率
            if Trade['OffsetFlag'] == '0':
                self.__a_open_count_strategy += Trade['Volume']  # A合约开仓手数
        # B合约的Trade
        elif Trade['InstrumentID'] == self.__b_instrument_id:
            ...
            if Trade['OffsetFlag'] == '0':
                self.__b_open_count_strategy += Trade['Volume']  # A合约开仓手数
        else:
    ...
    def count_instrument_action(self, OrderAction):
        # 撤单数量统计
    ...
    def Margin_Occupied_SHFE(self):
        ...
        for i in self.__list_position_detail_for_trade:
            ...
            if i['ExchangeID'] == 'S':  # 飞鼠柜台返回交易所代码上期所为"S"
        ...
        if len(list_commodity_id) == 1:
            ...
            # print(">>> Strategy.Margin_Occupied_SHFE() user_id =", self.__user_id, "strategy_id =", self.__strategy_id, "a合约和b合约是同一个品种 self.__Margin_Occupied_SHFE =", self.__Margin_Occupied_SHFE)
        # a合约和b合约是不同的品种
        elif len(list_commodity_id) == 2:
            # 分别选出两种持仓的明细
            ...
            self.__Margin_Occupied_SHFE = margin_0 + margin_1
            # print(">>> Strategy.Margin_Occupied_SHFE() user_id =", self.__user_id, "strategy_id =", self.__strategy_id, "a合约和b合约是不同的品种 self.__Margin_Occupied_SHFE =", self.__Margin_Occupied_SHFE)
    ...
    def get_instrument_margin_ratio(self, instrument_id):
        ...
        for i in self.__user.get_instrument_info():
            if i['InstrumentID'] == instrument_id:
                # print(">>>Strategy.get_instrument_margin_ratio() user_id =", self.__user_id, "strategy_id =", self.__strategy_id, "instrument_id =", instrument_id, i)
                found = True
    ...
    def get_a_open_count_strategy(self):
        return self.__a_open_count_strategy

    def get_b_open_count(self):
        return self.__b_open_count_strategy
    ...
    def get_statistics(self):
        dict_statistics = {
            ...
            'a_open_count': self.__a_open_count,  # A开仓手数
            'b_open_count': self.__b_open_count,  # B开仓手数
            'a_open_count_strategy': self.__a_open_count_strategy,  # 本策略A开仓手数
            'b_open_count_strategy': self.__b_open_count_strategy,  # 本策略B开仓手数
    ...
    # 飞鼠柜台的撤单回报回调
    def OnRspOrderAction(self, OrderAction):
        """报单操作请求响应:撤单操作响应"""
        # print(">>>Strategy.OnRspOrderAction() OrderAction =", OrderAction)
        self.count_instrument_action(OrderAction)
